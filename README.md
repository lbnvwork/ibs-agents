...
## Правила
- Общие `clinerules/10-git … 50-dod` — симлинки в каждый `ibs-*/.clinerules/`.
- Ролевой `roles/<role>/00-role.md` — симлинк в `ibs-*/.clinerules/00-role.md`.
...
## Обновление
Правка в `ibs-agents` → коммит (руководитель) → пуш (по отмашке) → агенты видят изменения сразу (симлинк), перезапуск Cline не требуется.
